...
void UMainToolbarWidget::LoadToolbarIcons()
{
    ...
    IconPhysics = UResourceManager::GetInstance().Load<UTexture>("Data/Icon/Toolbar_Physics.png");
...
void UMainToolbarWidget::RenderPhysicsAssetEditorButton()
{
    ...
    if (IconPhysics && IconPhysics->GetShaderResourceView())
    {
        if (ImGui::ImageButton("##PhysicsAssetEditorBtn", (void*)IconPhysics->GetShaderResourceView(), IconSizeVec))
        {
            bButtonClicked = true;
        }

        if (ImGui::IsItemHovered())
            ImGui::SetTooltip("Physics Asset Editor를 엽니다");
    }
    else
    {
        // 아이콘이 없을 경우 텍스트 버튼
        if (ImGui::Button("PA", ImVec2(IconSize + 8, IconSize + 8)))
        {
            bButtonClicked = true;
        }
