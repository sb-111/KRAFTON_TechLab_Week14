...
void UGizmoArrowComponent::Render(URenderer* Renderer, const FMatrix& View, const FMatrix& Proj)
{
    ...
    // 하이라이트 상수
    Renderer->GetRHIDevice()->SetUpdateConstantBuffer(HighLightBufferType(true, FVector(1, 1, 1), AxisIndex, bHighlighted ? 1 : 0, 0, 1));
    // 리사이징
    ...
    Renderer->GetRHIDevice()->SetUpdateConstantBuffer(ModelBufferType(M));
    Renderer->GetRHIDevice()->SetUpdateConstantBuffer(ViewProjBufferType(View, Proj));
